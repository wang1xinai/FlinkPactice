...
//          窗口中的所有数据会被缓存，当窗口被计算的时候，才会拿出来
//    分类：
//          增量聚合函数aggregation(来一条数据处理一条数据，最终输出累加器)（一个分区只维护一个累加器）
//                  .timeWindow(Time.seconds(5)).aggregate(new AggregateFunction())
//                  implements AggregationFunction<IN, ACC, OUT>
//          全窗口聚合函数(处理这个窗口的所有数据element)（带有窗口信息）
//                  .timeWindow(Time.seconds(5)).process(new ProcessWindowFunction())
//                  extends ProcessWindowFunction<IN, OUT, KEY, W extends Window>
//          增量聚合函数 + 全窗口聚合函数
//                  .timeWindow(Time.seconds(5)).aggregate(new AggregateFunction(),new ProcessWindowFunction())
...
public class WindowFunctionTest3 {
